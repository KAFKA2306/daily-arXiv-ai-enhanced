...
def perform_deduplication():
    ...
    try:
        ...
        print(
            f"历史{history_days}日去重库大小: {len(history_ids)} / History {history_days} days deduplication library size: {len(history_ids)}",
            file=sys.stderr,
        )
        ...
        if duplicate_ids:
            print(
                f"发现 {len(duplicate_ids)} 篇历史重复论文 / Found {len(duplicate_ids)} historical duplicate papers",
                file=sys.stderr,
            )
            ...
            if new_papers:
                if save_papers_data(new_papers, today_file):
                    print(
                        f"已更新今日文件，移除 {len(duplicate_ids)} 篇重复论文 / Today's file updated, removed {len(duplicate_ids)} duplicate papers",
                        file=sys.stderr,
                    )
                ...
                try:
                    ...
                    print(
                        "所有论文均为重复内容，已删除今日文件 / All papers are duplicate content, today's file deleted",
                        file=sys.stderr,
                    )
                except Exception as e:
...
def main():
    ...
    if dedup_status == "has_new_content":
        print(
            "✅ 去重完成，发现新内容，继续工作流 / Deduplication completed, new content found, continue workflow",
            file=sys.stderr,
        )
        sys.exit(0)
    elif dedup_status == "no_new_content":
        print(
            "⏹️ 去重完成，无新内容，停止工作流 / Deduplication completed, no new content, stop workflow", file=sys.stderr
        )
        sys.exit(1)
